...
'''
This function checks the DNS records for an internal (or public-facing) Application Load Balancer IP addresses.
It populates a Network Load Balancer's target group with Application Load Balancer's Internal IP addresses
...
NOTICE: If you are targeting an internet-facing ALB, the aws ec2 network interfaces API will be used to retrieve the internal IP addresses of the ALB.
...
def get_lb_scheme( lb_name ):
    """
    Get loadbalancer scheme
    :param lb_name the logical name of the load balancer
    :return: string of "internet-facing" or "internal"
    """
    theLB = elbv2client.describe_load_balancers( Names=[lb_name] )
    return theLB['LoadBalancers'][0]['Scheme']

def get_lb_internal_ip(lb_name):
    """
    If there are any spaces in your command's arguments this will fail!
    Uses ec2 API to retrieve internal IPs of the ALB.
    :param lb_name the logical name of the load balancer
    :return The IP internal addresses as a set
    """
    command = "/opt/aws ec2 describe-network-interfaces --filters Name=description,Values='*app/" + lb_name + "/*' --query NetworkInterfaces[*].PrivateIpAddresses[*].PrivateIpAddress --output text"
    command_list = command.split(' ')

    try:
        result = subprocess.run(command_list, capture_output=True)
        result = set(result.stdout.decode('UTF-8').replace(" ", "").splitlines())
        print(result)
        return result
...
def lambda_handler(event, context):
    ...
    lb_name = '.'.join(ALB_DNS_NAME.split('.')[:1])
    lb_name = '-'.join(lb_name.split('-')[:3])
    lb_scheme = get_lb_scheme( lb_name )
    ...
    elif ( lb_scheme == "internet-facing" ): 
        regular_record_set = get_lb_internal_ip( lb_name )
        print("INFO: IPs detected by awscli call:", regular_record_set)
        print("INFO: Number of IPs detected by awscli call: ", len(regular_record_set))
